...
@jit()
def masking_map(map1, nside, npix, limit, tmp):
    """
...
if __name__ == "__main__":
    ...
    Binary_mask = masking_map(Haslam_512, NSIDE, NPIX, LIMIT, 50.0)
    imp_map_50 = apodiz(Binary_mask, theta)

    LIMIT = 0.000122  # for gaussian 2 degree apodization 30K
    Binary_mask = masking_map(Haslam_512, NSIDE, NPIX, LIMIT, 30.0)
    imp_map_30 = apodiz(Binary_mask, theta)

    LIMIT = 0.000073
    Binary_mask = masking_map(Haslam_512, NSIDE, NPIX, LIMIT, 18.0)
    imp_map_18 = apodiz(Binary_mask, theta)
    """
    #LIMIT = 0.0002553  # 2 degree apodization
    LIMIT = 0.8553  # 2 degree apodization
    Binary_mask = masking_map(Haslam_512, NSIDE, NPIX, LIMIT, 200.0)
    #imp_map_200 = apodiz(Binary_mask, 5.0)
    ...
    """
    Haslam_200 = Haslam_512*Binary_mask #*imp_map_200
    LMAX = 300
    l = np.arange(0, 301)
    ...
    hp.mollview(Binary_mask, xsize=2000, unit=r'$T_{B}(K)$', nest=False)

    filename = '108K_Mask.fits'
    hp.fitsfunc.write_map(filename, Binary_mask)
    filename = '108K_Map.fits'
    hp.fitsfunc.write_map(filename, Haslam_200)

    """
    hp.mollview(imp_map_18, xsize=2000, unit=r'$T_{B}(K)$', nest=False)
    plt.savefig("/dataspace/sandeep/Bispectrum_data/Haslam_mask_512_18K.eps",
                dpi=100)
    ...
    plt.savefig("/dataspace/sandeep/Bispectrum_data/Haslam_mask_512_50K.eps",
                dpi=100)

    hp.mollview(imp_map_30, xsize=2000, unit=r'$T_{B}(K)$', nest=False)

    plt.savefig("/dataspace/sandeep/Bispectrum_data/Haslam_mask_512_200K.eps",
                dpi=100)

    hp.mollview(Haslam_18, xsize=2000, unit=r'$T_{B}(K)$', nest=False)

    plt.savefig("/dataspace/sandeep/Bispectrum_data/Haslam_map_18.eps",
                dpi=100)

    hp.mollview(Haslam_50, xsize=2000, unit=r'$T_{B}(K)$', nest=False)

    plt.savefig("/dataspace/sandeep/Bispectrum_data/Haslam_map_50.eps",
                dpi=100)
    """
   ...
   # plt.savefig("/dataspace/sandeep/Bispectrum_data/Haslam_map_108K_5deg_apodi.eps", dpi=100)
...
#    plt.plot(l, l * (l + 1) * cl_30, '-', color='green', linewidth=2, label='30K')
    ...
    #plt.savefig("/dataspace/sandeep/Bispectrum_data/Cl_108K_5deg_apodi.eps", dpi=100)
