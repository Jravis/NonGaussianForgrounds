...
Actual = np.zeros((5, lmax+1), dtype=np.float32)
Mean = np.zeros((5, lmax+1), dtype=np.float32)
Std_Dev = np.zeros((5, lmax+1), dtype=np.float32)

for fn in key:
    esti_cl = np.zeros((1000, lmax+1), dtype=np.float32)
    ...
    for i in xrange(0, 1000):
        s1 = "/dataspace/sandeep/Bispectrum_data"
        s2 = "/Gaussian_%s_test/Gaussian_%s_cl/haslam_%sgaussMap_cl_%d.txt" % (fn, fn, fn, i)
        filename = s1+s2
